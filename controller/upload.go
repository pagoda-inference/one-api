...
func UploadMediaFile(c *gin.Context) {
	...
	allowedExts := map[string]bool{".jpg": true, ".jpeg": true, ".png": true, ".gif": true, ".webp": true, ".pdf": true, ".mp4": true, ".mov": true, ".webm": true}
	if !allowedExts[ext] {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
	...
	objectKey := fmt.Sprintf("%d-%s", time.Now().UnixMilli(), filepath.Base(header.Filename))
	...
	fileURL := fmt.Sprintf("/api/images/%s", objectKey)
...
func GetMediaFile(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": fmt.Sprintf("file_not_found: %s", err.Error()),
		})
		return
	}
