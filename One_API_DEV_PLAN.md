...
### 3.9 P9 第九期 - 模型生态扩展（已完成）

**目标**：扩展支持的模型范围

| 任务 | 工期 | 优先级 | 负责角色 | 状态 |
|------|------|--------|----------|------|
| Provider 管理模块 | - | P9 | 全栈 | ✅ 已完成 |
| 模型管理模块 | - | P9 | 全栈 | ✅ 已完成 |
| 模型市场展示 | 5天 | P5 | 前端 | ✅ 已完成 |
| 模型试用功能 | 2天 | P9 | 全栈 | ✅ 已完成 |
| 模型 SLA 配置 | 1天 | P9 | 后端 | ✅ 已完成 |
...
**当前状态**：✅ 已实现 Token 级别和 Tenant 级别限流
...
```
┌─────────────────────────────────────────────────────────────┐
│  模型级别限流 (Model Rate Limit)     ← 待加                   │
│  设置者：Platform Admin                                      │
│  来源：上游 API 限制                                        │
│  字段：rate_limit_rpm, rate_limit_tpm                       │
│  目的：保护上游 API，不超发                                   │
└──────────────────────────┬──────────────────────────────────┘
                           │ min()
┌──────────────────────────▼──────────────────────────────────┐
│  公司级别限流 (Company Rate Limit)  ← 未来扩展                 │
│  设置者：Platform Admin (开通时) 或 Company Admin             │
│  来源：客户购买的配额                                        │
│  目的：控制客户账单上限                                       │
└──────────────────────────┬──────────────────────────────────┘
                           │ min()
┌──────────────────────────▼──────────────────────────────────┐
│  团队级别限流 (Team Rate Limit)   ← 已有                     │
│  设置者：Platform Admin                                      │
└──────────────────────────┬──────────────────────────────────┘
                           │ min()
┌──────────────────────────▼──────────────────────────────────┐
│  API Key 级别限流 (Token Rate Limit)  ← 已有                  │
└─────────────────────────────────────────────────────────────┘
```
...
```go
// 当前：独立检查
if tokenRpm > tokenLimit → 拒绝
if tenantRpm > tenantLimit → 拒绝

// 目标：级联取最小
effectiveRpm := min(modelLimit, companyLimit, teamLimit, keyLimit)
if tokenRpm > effectiveRpm → 拒绝
```
...
**当前实现状态**：✅ Tenant/Team 层级已完成，Company 层级待开发
...
| 角色 | 范围 | 职责 |
|------|------|------|
| Platform Admin | 平台级 | 模型管理、Provider管理、开除公司 |
| Company Admin | 公司级 | 管理团队、用户、设置公司限流 |
| Team Admin | 团队级 | 管理成员、设置团队限流 |
| User | 个人 | 创建API Keys、设置Key限流 |

**当前实现**：
- 只有 Tenant（团队）层级 ✅
- Company 层级暂无（等业务需要时再加）
- Platform Admin = root，最大权限 ✅
...
└──────────────────────────┬──────────────────────────────────┘
                           │ min()
┌──────────────────────────▼──────────────────────────────────┐
│  团队级别限流 (Team Rate Limit)                              │
│  设置者：Platform Admin 或 Company Admin                      │
│  来源：从公司配额分配                                         │
│  目的：部门独立核算                                          │
└──────────────────────────┬──────────────────────────────────┘
                           │ min()
┌──────────────────────────▼──────────────────────────────────┐
│  API Key 级别限流 (Token Rate Limit)                         │
│  设置者：User (自己)                                         │
│  来源：从团队配额分配                                         │
│  目的：用户在自己的额度内做资源分配                            │
└─────────────────────────────────────────────────────────────┘
...
| 层级 | 设置者 | 意义 |
|------|--------|------|
| 模型 | Platform Admin | 保护上游 API 限制 |
| 公司 | Platform/Company Admin | 控制客户账单上限 |
| 团队 | Company Admin | 部门独立核算 |
| Key | User | 用户在自己的额度内做资源分配 |
...
| P0 | 熔断器实现 | 2天 | ✅ 已完成 |
| P0-NEW | 模型级别限流 | 2天 | ⚠️ 待开发 |
| P0-NEW | 限流算法级联取最小值 | 1天 | ⚠️ 待开发 |
| P1 | 主动健康检查 | 3天 | ✅ 已完成 |
| P1 | 加权调度 | 2天 | ✅ 已完成 |
| P1 | 请求队列 | 1天 | ✅ 已完成 |
| P1 | RPM/TPM精细限流（Key级别） | 5~7天 | ✅ 已完成 |
| P2 | Batch批量推理 | 5~7天 | ✅ 已完成 |
| P3 | 监控指标暴露 | 3天 | ✅ 已完成 |
| P4 | 充值支付系统 | 10~15天 | 🔴 待开发 |
| P5 | 用户门户重构 | 10~15天 | ✅ 已完成 |
| P5 | 登录页面重构（Feishu SSO） | - | ✅ 已完成 |
| P5 | Provider管理模块 | - | ✅ 已完成 |
| P5 | 模型管理模块 | - | ✅ 已完成 |
| P5 | 团队管理模块 | - | ✅ 已完成 |
| P7 | 运营管理后台 | 8~10天 | ✅ 已完成 |
| P8 | 多租户权限（Tenant/Team） | 8~10天 | ⚠️ 部分完成 |
| P8 | Company层级 | - | 🔴 待开发 |
| P9 | Provider管理 | 持续 | ✅ 已完成 |
| P9 | 模型管理 | 持续 | ✅ 已完成 |
| P9 | 模型市场展示 | 持续 | ✅ 已完成 |
| P9 | 模型试用功能 | 2天 | ✅ 已完成 |
...
| v1.4 | 2026-04 | 更新状态：P5/P8/P9大部分已完成；新增限流体系完整设计（4级联）；新增用户分层架构设计 |