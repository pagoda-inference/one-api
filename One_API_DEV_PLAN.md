...
| 数据库 | MySQL 8.0 | One API 主库 + Batch 扩展表 |
...
| `middleware/queue.go` | 新增：请求队列 | 新建 |
| `monitor/health.go` | 新增：健康检查 | 新建 |
| `monitor/circuit_breaker.go` | 新增：熔断器 | 新建 |
| `monitor/scheduler.go` | 新增：加权调度器 | 新建 |
...
| 优先级 | 优化项 | 工期 | 收益 |
|--------|--------|------|------|
| P0 | 并发Bug修复 | 0.5天 | 避免生产panic |
| P0 | HTTP连接池优化 | 0.5天 | 提升吞吐 |
| P0 | 熔断器实现 | 2天 | 快速故障隔离 |
| P1 | 主动健康检查 | 3天 | 提前发现问题 |
| P1 | 加权调度 | 2天 | 更均衡负载 |
| P1 | 请求队列 | 1天 | 防止系统过载 |
| P2 | RPM/TPM限流 | 5~7天 | 精细化控制 |
| P3 | 最小并发调度 | 2天 | 长请求场景优化 |
